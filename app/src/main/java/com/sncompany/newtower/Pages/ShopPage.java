...
public class ShopPage extends TPage {
    ...
    public static final int BACK = 15, EQUIP = 16, ARROW_L = 17, ARROW_R = 18, BODY = 19, SIDEBAR = 20, SELL = 21, BTN_OK = 22, TOTAL = 23;
    ...
    public void paint_GAME_SHOP_SHOP(GL10 gl10, boolean init) {
        ...
        if (init) {
            ...
            if (buyFail == 0) { //Default StoremFront
                TouchManager.addTouchRectListData(BACK, GameRenderer.CGRectMake(11, 362, 68, 114));
                TouchManager.addTouchRectListData(EQUIP, GameRenderer.CGRectMake(19, 13, 42, 48));
                TouchManager.addTouchRectListData(ARROW_L, GameRenderer.CGRectMake(81, 397, 47, 48));
                TouchManager.addTouchRectListData(ARROW_R, GameRenderer.CGRectMake(672, 397, 47, 48));
                TouchManager.addTouchRectListData(BODY, GameRenderer.CGRectMake(30, 70, 690, 280));
                TouchManager.addTouchRectListData(SIDEBAR, GameRenderer.CGRectMake(722, 70, 52, 280));
                if (inventory.shopShopInventorySelectPos != -1)
                    TouchManager.addTouchRectListData(SELL, GameRenderer.CGRectMake(711, 381, 68, 78));
                for (int j = 0; j < 4; j++) {
                    int touch = (j * 2) + EquipPage.MIN_HERO;
                    int y = j * 70;
                    TouchManager.addTouchRectListData(touch, GameRenderer.CGRectMake(565, y + 75, 70, 60));
                    TouchManager.addTouchRectListData(touch + 1, GameRenderer.CGRectMake(636, y + 71, 78, 68));
                }
                inventory.addTouch();
            } else //BUy fail
                TouchManager.addTouchRectListData(BTN_OK, GameRenderer.CGRectMake(213, 259, 381, 65));
        ...
        uiShopImage[cTLS == BACK ? shop_btnbackon : shop_btnbackoff].drawAtPointOption(40, 13, 17);
        ...
        while (curBlock <= upgradeItemListDraw.totalHalfBlockSize + 3 + 1) {
            ...
            if (curInd != -1) {
                ...
                CGRect rect = GameRenderer.CGRectMake(30, 70, 690, 280);
                int r2 = (curBlock * 70) + upgradeItemListDraw.blockCorrectionPixel;
                uiShopImage[shop_shopitembar].drawAtPointOptionGuide(30, r2 + 70, 18, rect);
                inventory.drawUpItemImageGuide(curInd, 40, cY + upgradeItemListDraw.blockCorrectionPixel + 75, rect);
