...
public final class zzehm {
    ...
    public static ECParameterSpec zza(zzeho zzehoVar) throws NoSuchAlgorithmException {
        int i = zzehp.zziih[zzehoVar.ordinal()];
        if (i == 1) {
        ...
        String valueOf = String.valueOf(zzehoVar);
    ...
    public static ECPublicKey zza(zzeho zzehoVar, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ECParameterSpec zza = zza(zzehoVar);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        zza(eCPoint, zza.getCurve());
        return (ECPublicKey) zzeht.zziiw.zzhy("EC").generatePublic(new ECPublicKeySpec(eCPoint, zza));
    }

    public static KeyPair zza(ECParameterSpec eCParameterSpec) throws GeneralSecurityException {
        KeyPairGenerator zzhy = zzeht.zziiv.zzhy("EC");
        zzhy.initialize(eCParameterSpec);
        return zzhy.generateKeyPair();
    }
    ...
    public static byte[] zza(ECPrivateKey eCPrivateKey, ECPoint eCPoint) throws GeneralSecurityException {
        ...
        PublicKey generatePublic = zzeht.zziiw.zzhy("EC").generatePublic(new ECPublicKeySpec(eCPoint, eCPrivateKey.getParams()));
        KeyAgreement zzhy = zzeht.zziiu.zzhy("ECDH");
        zzhy.init(eCPrivateKey);
        try {
            zzhy.doPhase(generatePublic, true);
            byte[] generateSecret = zzhy.generateSecret();
